...
module.exports = eleventyConfig => {
  ...
  eleventyConfig.addTransform('line-numbers', (content, outputPath) => {
    if (!outputPath || !outputPath.endsWith('.html')) {
      return content
    }

    const $ = cheerio.load(content)
    $('[data-auto-animate] pre code').each((i, c) => {
      $(c).attr('data-id', 'foundation')
      $(c).attr('data-line-numbers', true)
      $(c).attr('data-trim', true)
    })
  ...
  eleventyConfig.addPassthroughCopy({
    './node_modules/reveal.js/dist/*.css': 'css',
    './node_modules/reveal.js/dist/*.js': 'js',
